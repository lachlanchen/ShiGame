...
namespace SHI
{
    public static class ShiUiText
    {
        ...
        {
            ["en"] = new[] { "Field guide", "Before the first order", "Every order resolves in three strokes: your move, the position’s answer, and the field condition shown in advance.", "Read the field", "Grain, trust, momentum, people, and exposure are different forms of power. No single meter is victory.", "Make your move", "A card shows exact immediate effects and the strategic form you are trying to create.", "Expect an answer", "The forecast warns what you expose. After commitment, the position answers and its deltas are recorded separately.", "Controller ready", "Controller supported", "D-pad/stick select · A/Cross commit · B/Circle close · shoulders open ledgers · Start guide", "Read the position", "New chronicle", "Field signal", "Chronicle seed", "Field condition resolves", "Your decisions will be impressed here like marks in wet clay." },
            ["ar"] = new[] { "دليل الميدان", "قبل إصدار الأمر الأول", "يُحسم كل أمر في ثلاث ضربات: حركتك، وردّ الموقف، وظرف الميدان المعروض مسبقًا.", "اقرأ الميدان", "المؤن والثقة والزخم والناس والانكشاف أشكال مختلفة للقوة. لا يعني مقياس واحد النصر.", "اصنع حركتك", "تعرض البطاقة الآثار الفورية الدقيقة والشكل الاستراتيجي الذي تحاول صنعه.", "توقّع الرد", "ينبهك التوقع إلى ما تكشفه. بعد الالتزام يرد الموقف وتُسجل تغيراته منفصلة.", "وحدة التحكم جاهزة", "وحدة التحكم مدعومة", "العصا/الأسهم: اختيار · A/×: تأكيد · B/○: إغلاق · الكتفان للسجلات · Start للدليل", "اقرأ الموقف", "سجل جديد", "إشارة الميدان", "بذرة السجل", "يُحسم ظرف الميدان", "ستُطبع قراراتك هنا كعلامات في طين مبتل." },
            ["de"] = new[] { "Feldführer", "Vor dem ersten Befehl", "Jeder Befehl wird in drei Schritten aufgelöst: dein Zug, die Antwort der Lage und die vorher gezeigte Feldbedingung.", "Lies das Feld", "Getreide, Vertrauen, Dynamik, Menschen und Entdeckung sind verschiedene Formen von Macht. Kein einzelner Wert bedeutet Sieg.", "Setze deinen Zug", "Eine Karte zeigt die genauen Sofortwirkungen und die strategische Form, die du schaffen willst.", "Erwarte eine Antwort", "Die Prognose warnt vor deiner offenen Flanke. Nach der Bindung antwortet die Lage; ihre Änderungen werden getrennt notiert.", "Controller bereit", "Controller unterstützt", "Steuerkreuz/Stick wählen · A/Kreuz bestätigen · B/Kreis schließen · Schultertasten öffnen Register · Start öffnet den Führer", "Lage lesen", "Neue Chronik", "Feldsignal", "Chronik-Seed", "Feldlage wirkt", "Deine Entscheidungen werden hier wie Spuren in nassem Ton geprägt." },
            ["es"] = new[] { "Guía de campo", "Antes de la primera orden", "Cada orden se resuelve en tres pasos: tu movimiento, la respuesta de la posición y la condición del terreno mostrada de antemano.", "Lee el campo", "Grano, confianza, impulso, pueblo y exposición son formas distintas de poder. Ningún indicador por sí solo es la victoria.", "Haz tu movimiento", "Una carta muestra los efectos inmediatos exactos y la forma estratégica que intentas crear.", "Espera una respuesta", "El pronóstico advierte qué dejas expuesto. Tras comprometerte, la posición responde y sus cambios quedan registrados aparte.", "Mando listo", "Compatible con mando", "Cruceta/palanca para elegir · A/Cruz confirma · B/Círculo cierra · botones superiores abren registros · Start abre la guía", "Leer la posición", "Nueva crónica", "Señal del terreno", "Semilla de crónica", "Se resuelve la condición del terreno", "Tus decisiones quedarán aquí como marcas en arcilla húmeda." },
            ["fr"] = new[] { "Guide de terrain", "Avant le premier ordre", "Chaque ordre se résout en trois temps : votre coup, la réponse de la position et la condition du terrain annoncée à l’avance.", "Lisez le terrain", "Grain, confiance, élan, peuple et exposition sont des formes distinctes de pouvoir. Une seule jauge ne signifie jamais la victoire.", "Jouez votre coup", "Une carte montre les effets immédiats exacts et la forme stratégique que vous tentez de créer.", "Attendez une réponse", "La prévision signale ce que vous exposez. Après l'engagement, la position répond et ses écarts sont consignés séparément.", "Manette prête", "Manette prise en charge", "Croix/stick pour choisir · A/Croix confirme · B/Rond ferme · boutons supérieurs ouvrent les registres · Start ouvre le guide", "Lire la position", "Nouvelle chronique", "Signal du terrain", "Graine de chronique", "La condition du terrain s'applique", "Vos décisions s'imprimeront ici comme des marques dans l'argile humide." },
            ["ja"] = new[] { "戦場案内", "最初の命令の前に", "一つの命令は三段階で決着する。自分の一手、局面の応手、そして事前に示された戦場条件だ。", "場を読む", "兵糧、信頼、勢い、民衆、露見はそれぞれ異なる力の形だ。一つの値だけで勝利は決まらない。", "一手を打つ", "札には即時の正確な変化と、作ろうとする戦略的な形が示される。", "応手を待つ", "予兆は露出する弱みを告げる。決断後、局面が応じ、その変化は別に記録される。", "コントローラー準備完了", "コントローラー対応", "十字/スティックで選択 · A/×で決定 · B/○で閉じる · 肩ボタンで台帳 · Startで案内", "局面を読む", "新しい記録", "戦場の兆し", "記録シード", "戦場条件の反映", "決断は濡れた粘土の跡のようにここへ刻まれます。" },
            ["ko"] = new[] { "전장 안내", "첫 명령을 내리기 전에", "하나의 명령은 세 단계로 결산된다. 나의 수, 국면의 응수, 그리고 미리 공개된 전장 조건이다.", "판을 읽어라", "군량, 신뢰, 기세, 민심, 노출은 서로 다른 힘의 형태다. 하나의 수치만으로 승리할 수 없다.", "수를 두어라", "선택지는 즉시 일어나는 정확한 변화와 만들려는 전략적 형세를 보여 준다.", "응수를 예상하라", "예고는 드러나는 약점을 알린다. 결단 뒤 국면이 응수하며 그 변화는 따로 기록된다.", "컨트롤러 준비됨", "컨트롤러 지원", "십자/스틱 선택 · A/× 결정 · B/○ 닫기 · 숄더 버튼으로 장부 · Start로 안내", "국면 읽기", "새 기록", "전장 신호", "기록 시드", "전장 조건 반영", "결정은 젖은 진흙 자국처럼 이곳에 새겨집니다." },
            ["ru"] = new[] { "Полевое руководство", "Перед первым приказом", "Каждый приказ разрешается в три этапа: ваш ход, ответ позиции и заранее показанное условие поля.", "Прочтите поле", "Зерно, доверие, порыв, люди и раскрытие — разные формы власти. Ни один показатель сам по себе не означает победу.", "Сделайте ход", "Карточка показывает точные немедленные эффекты и стратегическую форму, которую вы пытаетесь создать.", "Ждите ответа", "Прогноз предупреждает, что вы открываете. После решения позиция отвечает, а её изменения записываются отдельно.", "Геймпад готов", "Геймпад поддерживается", "Крестовина/стик — выбор · A/Крест — решение · B/Круг — закрыть · бамперы открывают записи · Start открывает руководство", "Прочесть позицию", "Новая хроника", "Сигнал поля", "Код хроники", "Условие поля разрешается", "Решения отпечатаются здесь, как знаки на сырой глине." },
            ["vi"] = new[] { "Cẩm nang chiến trường", "Trước mệnh lệnh đầu tiên", "Mỗi mệnh lệnh được kết toán qua ba bước: nước của bạn, thế cục đáp lại và điều kiện bàn thế đã báo trước.", "Đọc bàn thế", "Lương, tín, thế, dân và bại lộ là những dạng quyền lực khác nhau. Không một chỉ số nào tự nó là chiến thắng.", "Đi nước của bạn", "Mỗi thẻ cho biết thay đổi tức thời chính xác và thế chiến lược bạn muốn tạo.", "Chờ thế cục đáp lại", "Dự báo chỉ ra điểm bạn để lộ. Sau khi cam kết, thế cục đáp lại và biến đổi của nó được ghi riêng.", "Tay cầm đã sẵn sàng", "Có hỗ trợ tay cầm", "D-pad/cần để chọn · A/Dấu chéo xác nhận · B/Vòng tròn đóng · nút vai mở sổ · Start mở hướng dẫn", "Đọc thế cục", "Biên niên mới", "Tín hiệu bàn thế", "Hạt giống biên niên", "Điều kiện bàn thế được áp dụng", "Quyết định sẽ in tại đây như dấu trên đất sét ướt." },
            ["zh-Hans"] = new[] { "观势入门", "第一道命令之前", "每道命令分三段结算：你的落子、局势的应手，以及事先明示的局势条件。", "先读全局", "粮、信、势、民、险是五种不同的力量。任何一个数值都不等于胜利。", "再落一子", "选择牌会显示准确的即时变化，以及你试图塑造的战略形势。", "预判应手", "压力预兆指出你暴露的弱点。承诺之后，局势会应手，其变化会被单独记录。", "手柄已就绪", "支持手柄", "方向键/摇杆选择 · A/叉确认 · B/圈关闭 · 肩键打开简牍 · Start打开入门", "开始观势", "重开一卷", "局势信号", "本局种子", "局势条件结算", "你的选择会像湿泥上的刻痕一样留在这里。" },
