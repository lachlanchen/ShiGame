...
4. Press Play. `ShiBootstrap` creates the 3D wartable, visual rain field, map markers, opt-in procedural soundscape, and game UI at runtime.

The native input baseline uses the committed legacy-input axis map for broad compatibility: arrows or left stick/D-pad move across enabled orders without changing campaign state; clicking a compact order also selects it; the complete selected-order reading stays visible; and the separate **Issue order** action, Enter/Space or the south face button commits through one resolver. Escape/east face button closes, shoulders open record/sources, and Start/Menu opens the field guide. `M` or Y/Triangle enters wartable inspection; arrows/D-pad/stick cycle known, reported and reference-only sites; confirm opens that site's evidence; and clicking a 3D marker uses camera raycasting. The one-time guide preference is separate from campaign saves. Guide/decision/record/controller/map/audio text exists in all eleven UI locales. Sound starts off; the header mixer persists independently capped ambience/effects buses and previews the shared semantic commit cue.
...
`SHI/Validate Production Content` exposes the same preflight in the editor. It checks cross-engine ASCII IDs, cycles and reachability, site/speaker/source references, intelligence status and coordinate bounds, site claim/source closure, source/commitment/field/opponent classifications, establishing-choice and outcome closure, complete non-overlapping opponent bands, strategic-method/read closure, adverse counter effects, resource and requirement bounds, pressure responses, bounded commitment/pursuit/method/field effects, baseline English and Simplified Chinese text, three authored endings, and the exact audio schema/defaults/mix/envelope/quality ranges. It reconstructs deterministic rain to reject DC offset and an abnormal loop-boundary transient; EditMode coverage additionally checks every shared cue against the authored audibility floor. The runtime uses the shared FNV-1a seed contract, applies action → disclosed commitment answer → pressure → disclosed pursuit → disclosed method read → disclosed field effects, replays save-v6 history while preserving v1–v3 decisions before pursuit, v1–v4 decisions before method reads and v1–v5 decisions before commitments, records failure consistently with the TypeScript core, and routes pointer/keyboard/controller commitment through the same resolver. Successful player builds write a receipt under `apps/unity/Builds/`; local builds and test output remain ignored by Git.
