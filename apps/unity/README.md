...
`SHI/Validate Production Content` exposes the same preflight in the editor. It checks cross-engine ASCII IDs, cycles and reachability, site/speaker/source references, intelligence status and coordinate bounds, site claim/source closure, source/field/opponent classifications, complete non-overlapping opponent bands, strategic-method/read closure, adverse counter effects, resource and requirement bounds, pressure responses, bounded pursuit/method/field effects, baseline English and Simplified Chinese text, three authored endings, and the exact audio schema/defaults/mix/envelope/quality ranges. It reconstructs deterministic rain to reject DC offset and an abnormal loop-boundary transient; EditMode coverage additionally checks every shared cue against the authored audibility floor. The runtime uses the shared FNV-1a seed contract, applies action → pressure → disclosed pursuit → disclosed method read → disclosed field effects, replays save-v5 history while preserving v1–v3 decisions before pursuit and v1–v4 decisions before method reads, records failure consistently with the TypeScript core, and routes pointer/keyboard/controller commitment through the same resolver. Successful player builds write a receipt under `apps/unity/Builds/`; local builds and test output remain ignored by Git.
