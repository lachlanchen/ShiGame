...
This is the priority Unreal Engine 5.8 C++ client for SHI. It consumes the same canonical schema-v7 campaign and Broken Crossing engagement contract as Web and Unity. Epic's official Linux `5.8.1` installed build generates the project, compiles/links `SHIEditor` and passes the exact eleven-suite `SHI.` native automation namespace. A fixed-window standalone route is visibly playable through noVNC; stable editor PIE, packaged-player and human-playability claims still require the recorded gates below.
...
Official UE `5.8.1` changelist `56057345` project generation and `SHIEditor` compilation/linking pass on this workstation. An exact `SHI.` run discovers eleven suites and passes all eleven: procedural audio, 46-route campaign replay, fail-closed order transaction, save/replay integrity, schema-v7 horizon, council staging, consequence grammar, live command signals, native Broken Crossing parity, historical source/claim closure and wartable spatial intelligence. Tests run with `-nowrite` so commandlets cannot modify tracked project configuration. A localhost-only noVNC run visibly completes all three tactical pulses, shows six simultaneous 3D tallies, reaches an outcome, returns to council and preserves the campaign save hash. The stable route is currently fixed-window standalone with render threads disabled; editor PIE hit an NVIDIA Vulkan outdated-swapchain crash, so stable PIE, normal-thread performance, film quality, a packaged player, audible device output and human playability remain red. Transaction failure injection, full site/signal/figure/evidence/controller review, consequence motion/lens/reduced-motion review, audio capture/listening, final asset import and art-direction parity also remain open. See the [engagement command-space contract](../../docs/design/UNREAL_ENGAGEMENT_COMMAND_SPACE.md), [order-transaction contract](../../docs/design/UNREAL_ORDER_TRANSACTION.md), [canonical council-staging contract](../../docs/design/UNREAL_COUNCIL_STAGING.md), [command-space signal contract](../../docs/design/COMMAND_SPACE_SIGNALS.md) and [consequence-cinema contract](../../docs/design/UNREAL_CONSEQUENCE_CINEMA.md).
