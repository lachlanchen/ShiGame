...
- pointer marker picking, `Tab`/Gamepad RB site cycling, `Shift+Tab` reverse cycling and `Home` return to the current playable ground;
...
- schema-v7, spatial-wartable, source/claim closure, procedural-audio, 46-route conformance and save-integrity automation source;
- content sync, static project validation and official editor/build/test/package wrapper.

The current source slice implements the canonical six-layer decision order, advances all six authored nodes and reads the same source/claim truth as the other clients. Its evidence validator rejects orphan records, claim/source drift, rights mismatch, private paths, unregistered public-link origins and reconstruction disguised as historical evidence. Wartable focus uses a short eased camera transition; the order-response beat is calculated from an explicit base transform and returns to it exactly instead of accumulating camera drift. Engine-native marker assets are explicitly cooked and stencil rendering is configured. The first native sound path also exists in source: sound starts silent, a remembered preference is only armed until a player command, `M`/Gamepad Y and the Slate control toggle it, and rain/cue levels persist independently. It does **not yet claim native execution**, film quality, a packaged player, audible Unreal output or human playability: the C++ suites are authored but cannot execute until the official engine is installed. Native marker/evidence interaction, audio capture/listening, higher-order camera grammar, final asset import and art-direction parity remain open gates.
