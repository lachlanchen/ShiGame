...
- one original fictional command-weight production blockout under `Content/SHI/Art/Props/CommandWeight`: exact centimeter scale, 3,256/1,384-triangle LODs, UV0/lightmap UV1, two material slots, convex collision, forced cooking and a separately verified 499-package launch; it is admitted but not yet placed or final-shaded;
...
Official UE `5.8.1` changelist `56057345` project generation and `SHIEditor` compilation/linking pass on this workstation. An exact `SHI.` run discovers eleven suites and passes all eleven: procedural audio, 46-route campaign replay, fail-closed order transaction, save/replay integrity, schema-v7 horizon, council staging, consequence grammar, live command signals, native Broken Crossing parity, historical source/claim closure and wartable spatial intelligence. Tests run with `-nowrite` so commandlets cannot modify tracked project configuration. The official Linux package path exits 0 after compile/cook/stage/archive; its campaign and engagement StreamingAssets exactly match the canonical hashes. The archived normal-thread player visibly advances to Broken Crossing, completes all three tactical pulses, shows six simultaneous 3D tallies, reaches an outcome, returns to council and preserves the packaged campaign save hash before/during/after the preview. It then completes and cold-replays the sealed four-decision chapter. A real 60 Hz display FPS chart records 195.18 FPS average, 2.73 ms GPU, 4.11 ms render thread, 1.62 ms game thread and zero hitches; the slower noVNC rate is isolated to Xvfb/capture presentation. Real unwritable-save, failed-restart, missing-council and missing-command-signal attacks all visibly hold state before commit. Cuts-only consequence/skip/restart persistence passes, and direct PipeWire capture proves digital silence before opt-in/after disable plus bounded active native output.

The first reviewed authored prop also completes its isolated engine loop: Blender validation, exact Unreal import, 499-package cook-content proof and archived-player launch all pass. This is package admission, not runtime placement or final shading. Editor PIE still hits an NVIDIA Vulkan outdated-swapchain crash, so stable PIE, film quality and human playability remain red. Physical-controller review, natural-motion comfort, human audio listening, final character/environment art, command-weight PBR/council-lens review and overall art-direction parity also remain open. See the [runtime acceptance evidence](../../docs/production/evidence/unreal-runtime-acceptance-status.json), [package evidence](../../docs/production/evidence/unreal-linux-package-status.json), [command-weight admission evidence](../../docs/production/evidence/unreal-command-weight-import-status.json), [engagement command-space contract](../../docs/design/UNREAL_ENGAGEMENT_COMMAND_SPACE.md), [order-transaction contract](../../docs/design/UNREAL_ORDER_TRANSACTION.md), [canonical council-staging contract](../../docs/design/UNREAL_COUNCIL_STAGING.md), [command-space signal contract](../../docs/design/COMMAND_SPACE_SIGNALS.md) and [consequence-cinema contract](../../docs/design/UNREAL_CONSEQUENCE_CINEMA.md).
