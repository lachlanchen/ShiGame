...
Pre-alpha objective-audio-engineering checkpoint, 2026-08-09. Exact implementation `4dd6a520fe7194c2d57124e8eebf1b68583395b6` passes a detached clean `npm ci`/build, 177 visible gameplay/accessibility/localization checks and an independent eight-check actual-output capture with zero console errors. The deterministic reference passes nine peak/loudness/cue/DC/loop/channel gates at −36.7 LUFS/−23.0 dBTP; visible Chrome proves exact pre-consent digital silence and passes at −35.7 LUFS/−22.7 dBTP after the first real recording exposed and drove correction of a full-gain startup fade. The same local run preserves twenty-six semantic scans, eleven locale-font/layout audits, eighteen target audits and three UI-audio audits. The initial player code remains within its unchanged 100 KiB cap at 98.67 KiB gzip; 9.20 KiB CSS, 178.95 KiB largest lazy JavaScript, 22.94 MiB complete fonts and 26.77 MiB deployable output also pass enforced caps. A real Unity editor with Linux and Web support is installed, and the expanded Unity runtime/editor/EditMode-test sources—including zero-mean rain, shared cue envelopes and quality preflight—pass an offline warning-as-error reference-assembly compile. Native import, EditMode execution and player builds still require Unity account licensing; human listening/sensory-load/physical-device/native-audio review, assistive-technology and physical-controller sessions, observed players, and Qin-law/historical-GIS specialists remain open. CI/Pages and public-output replay are the next evidence boundary. Those gates remain red rather than hidden. SHI is planned as a one-year quality-driven production, and this repository will not label the goal complete until both clients, research, localization, assets, observed player sessions, and release checks pass.
