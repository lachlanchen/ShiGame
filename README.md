...
| [`apps/unreal`](apps/unreal/) | Priority Unreal Engine 5.8 C++ cinematic client: canonical schema-v7/audio loaders, presentation-independent deterministic session, 46-route golden replay suite, fail-closed save/resume, keyboard/gamepad command surface, six-layer resolution, all-scene progression, art-directed runtime command space, skippable camera beat, opt-in procedural rain/cues with persisted mix controls and official Linux build wrapper; native compile/play/audio evidence remains open until the official installed build is obtained |
