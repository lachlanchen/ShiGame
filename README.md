...
- At the broken crossing, the selected doctrine opens a three-pulse command exercise: issue only legal orders, read the field's separate answer, manage six local states and carry the resulting cost back to council. It is a validated Web reference until native Unreal reproduces the same replay.
...
Pre-alpha schema-v7/Unreal-priority checkpoint, 2026-08-09. Exact Web command-board implementation `d872be2606918521cb91d5c0ceb9eb4ef6849a94` makes the shared broken-crossing contract playable as a deliberately non-authoritative three-pulse exercise: the selected campaign plan and disclosed field condition determine legal commands, each order receives a separate authored response, six local meters change visibly, and the completed record previews—but does not write—campaign effects. The complete production build passes 57 TypeScript/UI tests, all 76 engagement routes, 689 successful and 87 capture/scattering campaign condition routes, the 46-route cross-engine corpus, static Unreal/history/audio/accessibility/font/privacy gates and every bundle budget. Its SHA-pinned visible noVNC route passes 264 checks across the full campaign plus pointer/keyboard/standard-gamepad command play, desktop/mobile layouts, eleven locales, thirty-five axe states, twenty-seven target states, forced colors, 320px reflow and actual Chrome 400% zoom, with zero console errors or remote/failed requests. The engagement remains **not campaign-authoritative** until native Unreal replays it identically. The native baseline at canonical council-staging implementation `5764a2dc7071ea2355214ae1aec6f9922e1a2a65` loads the shared character register and per-node speaker, rejects unknown/duplicate cast or missing dialogue, derives an exact speaker/Keeper stage with a 44° dialogue camera, labels historical words as authored dramatization and fictional speakers as reconstruction, and drives two persistent clickable body/head/mantle performance proxies. `D`, Gamepad R3, Slate or a figure click restores the non-mutating council shot; every consequence completes or skips into the next canonical speaker. Its copied-session, independent-replay, live-actor and durable-first order/restart transaction binds the save, rule record, selection, nine signals, council stage and consequence cinema; any cast, dialogue, disclosure, blocking, lens, world, cinema, actor or save drift holds the order and preserves the chronicle. The initial player remains inside the caps at 99.82 KiB JavaScript, 11.94 KiB CSS, 178.95 KiB largest lazy JavaScript, 22.94 MiB fonts and 26.88 MiB deployable output. The official 5.8.1 archive is currently being acquired through an outside-Git private supervisor, but Unreal is **not yet claimed native-playable, final-character quality or film quality**: native compile/automation, PIE figure/collision/failure injection, command-board implementation, interaction/handoff/motion/lens/reduced-motion review, final reviewed character import, Linux packaging, performance and human review remain red. Unity remains a maintained baseline with its existing license gate. Human listening, assistive-technology, physical-controller, observed-player, Qin-law, geography and translation review also remain open. SHI is a one-year quality-driven production; no source-only engine boundary is allowed to masquerade as a shipped game.
