...
Pre-alpha deliberate-order checkpoint, 2026-08-09. Exact implementation `b7531c90917485dc0ea613c23925198593ea60f5` passes a fresh-clone install and the complete local build/test/content-route/accessibility gates; the isolated noVNC browser repeats 231 checks against that same commit with zero console errors, cross-origin requests/resources or failed requests. Hosted validation run `31290543915` and Pages deployment run `31290543922` pass at evidence boundary `3f7cb8fca40465e5305ef9bb8b9d4e49292af60e`, and the public game repeats all 231 checks with zero console errors, cross-origin requests/resources or failed requests. Decision cards are now calm select-only summaries: pointer or `Shift+1`–`Shift+3` changes the focused order without mutating resources, history or save state. A complete inspector then exposes the selected order's strategy, method hit/miss, named promise, pressure and exact effects; only the separate, order-named **Issue order** control commits it. This flow is implemented in all eleven Web and Unity UI locales, remains usable on mobile and under actual Chrome 400% zoom, and preserves controller confirmation through the same resolver. Save v6, six-layer resolution, 689 successful/87 capture-scattering routes, all three commitments and all nine authored outcomes remain intact. The initial player stays inside the caps at 99.32 KiB JavaScript and 11.94 KiB CSS gzip; 178.95 KiB largest lazy JavaScript, 22.94 MiB complete fonts and 26.83 MiB deployable output also pass. The visible run covers thirty-two axe states, twenty-four target states, eleven locale/font states and three audio-UI states. Unity runtime, editor-validation and EditMode-test sources—including deliberate selection/confirmation and localized decision readings—compile as separate warning-clean assemblies against the installed Unity 2022.3 reference set. Native import, EditMode execution and player builds still require Unity account licensing; human listening/sensory-load/physical-device/native-audio review, assistive-technology and physical-controller sessions, observed players, and Qin-law/historical-GIS specialists remain open. Those gates remain red rather than hidden. SHI is planned as a one-year quality-driven production, and this repository will not label the goal complete until both clients, research, localization, assets, observed player sessions, and release checks pass.
