...
| [`apps/unreal`](apps/unreal/) | Priority Unreal Engine 5.8 C++ cinematic client: canonical schema-v7/audio/edition loaders, fail-closed source/claim/rights validation, presentation-independent deterministic session, 46-route golden replay suite, durable-first order/restart transactions, six-layer all-scene progression, bounded five-site wartable plus nine live resource/field/pursuit/method/oath 3D signals with pointer/keyboard/gamepad inspection, scoped historical basis, and a six-or-seven-beat consequence grammar that binds exact resolved layers to live actors, chooses safe cut/ease motion and 40°–58° semantic lenses, stays below five seconds, locks commands, supports persistent cuts-only reduced motion and skips back to authoritative ground; opt-in procedural rain/cues, persisted mix controls and the official Linux build wrapper are source-authored, while native compile/play/audio evidence remains open until the official installed build is obtained |
...
Pre-alpha schema-v7/Unreal-priority checkpoint, 2026-08-09. Exact fail-closed order implementation `f866d32f45d14afb7241de7e70c781f93b79a09f` passes a detached clean install and complete build: 47 TypeScript/UI tests, 689 successful and 87 capture/scattering condition routes, the regenerated 46-route cross-engine corpus, synchronized Web/Unreal/Unity payloads, Unreal project preflight, accessibility/audio/font/privacy gates and all bundle budgets. Unreal now prepares each order on a copied deterministic session, requires exactly one decision, rebuilds all nine 3D signals, authors the complete consequence sentence, independently recomputes and exactly compares rule/world/camera output, checks every live actor, and durably writes the candidate save before swapping active memory. Any rule, world, cinema, actor or save failure holds the order and preserves the current chronicle; two-step restart has the same durable-first boundary. Every turn of all 46 golden routes uses this transaction in authored native automation. The cinematic command space still binds exact order/oath/pressure/pursuit/method/field/position records only to live actors, completes in 3.52–4.08 seconds, cuts unknown/far targets, eases only within 100 Unreal units/6°, carries fixed 40°–58° semantic lenses, blocks unrelated commands, offers persisted cuts-only reduced motion and skips to authoritative ground. Hosted validation `31299721604` and Pages deployment `31299721605` pass on documented checkpoint `763ff05f2b9ebcf9a43fab1d2c0c030c96b1bc83`; its local, tracking and remote Git identities matched, and the Web deployment returned HTTP 200 before this evidence-note commit. The last complete visible-browser campaign run remains pinned to `a11e173a58d9290f1368f1596e730a13ccb5e820` with 248 checks across all acts/scenes, desktop/mobile, eleven locales, forced colors, 320px reflow and actual Chrome 400% zoom. The initial player remains inside the caps at 99.47 KiB JavaScript, 11.94 KiB CSS, 178.95 KiB largest lazy JavaScript, 22.94 MiB fonts and 26.84 MiB deployable output. Unreal is **not yet claimed native-playable or film quality**: the transaction, cinematic, spatial, command-signal and route C++ tests are authored, but official editor acquisition, native compile/automation, PIE success/failure injection, interaction/motion/lens/reduced-motion review, Linux packaging, performance and human review remain red. Unity remains a maintained baseline with its existing license gate. Human listening, assistive-technology, physical-controller, observed-player, Qin-law, geography and translation review also remain open. SHI is a one-year quality-driven production; no source-only engine boundary is allowed to masquerade as a shipped game.
