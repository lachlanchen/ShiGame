...
| [`apps/unreal`](apps/unreal/) | Priority Unreal Engine 5.8 C++ cinematic client: canonical schema-v7/audio/edition loaders, fail-closed source/claim/rights validation, presentation-independent deterministic session, 46-route golden replay suite, fail-closed save/resume, six-layer all-scene progression, bounded five-site wartable plus nine live resource/field/pursuit/method/oath 3D signals with pointer/keyboard/gamepad inspection, scoped historical basis, and an atomic six-or-seven-beat consequence grammar that binds exact resolved layers to live actors, stays below five seconds, locks commands and skips back to authoritative ground; opt-in procedural rain/cues, persisted mix controls and the official Linux build wrapper are source-authored, while native compile/play/audio evidence remains open until the official installed build is obtained |
