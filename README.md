...
Pre-alpha historical-production-cell checkpoint, 2026-08-09. Schema v3 now exposes seven source records, thirteen playable claim records and five registered editions with exact locators, uncertainty, rights metadata and unresolved specialist states. The web chapter retains authored pressure responses, disclosed seed-selected field conditions, replayable save-v3 migration, localized field guide, keyboard/standard-gamepad/touch operation, and exhaustive condition-route checks. Localhost and the public Pages build both pass the expanded 56-check visible gate with zero console errors for implementation `1e89bec` and deployment/evidence boundary `c709726`. A real Unity editor with Linux and Web support is installed and visible in Hub, and the expanded Unity runtime, editor and EditMode-test sources pass an offline reference-assembly type compile. Official project import, EditMode execution, physical-controller sessions and player builds remain stopped by Unity account licensing or require observed hardware; Qin-law and historical-GIS specialist gates also remain open. Those gates remain red rather than hidden. SHI is planned as a one-year quality-driven production, and this repository will not label the goal complete until both clients, research, localization, assets, observed player sessions, and release checks pass.
