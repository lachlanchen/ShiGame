...
| [`apps/unreal`](apps/unreal/) | Priority Unreal Engine 5.8 C++ cinematic client: official UE 5.8.1 native compile and fourteen-suite automation; canonical schema-v7/audio/edition/character loaders; independent 46-route campaign and 76-route command replay; fail-closed durable-first order/restart transactions; complete six-scene progression; bounded wartable/signals, consequence cinema and opt-in sound; plus reviewed original command-weight, fictional earth/wood command-ground and `24 × 24 m` wet-field production blockouts. The 505-package player visibly advances Act I → Act III, completes the three-pulse engagement and returns the campaign unchanged. Shelter/rain/vegetation, period figures/formations and final lighting remain open. |
...
| [`assets`](assets/) | Reviewed Daze/Broken Crossing lookdev plus AgenticApp/Blender wartable, command-weight, command-surface and wet-field sources, previews, `.blend`, GLB/FBX LODs, rejected-trial records and SHA-256 provenance |
...
Epic's official UE 5.8.1 installed build compiles and links the C++ project and passes exactly fourteen native suites against the byte-identical campaign and engagement payloads. The archived normal-thread Linux Development player visibly completes the three-pulse Broken Crossing exercise without mutating campaign authority, then separately completes, seals and cold-resumes the full four-decision chapter. Real packaged attacks against save writes, restart writes, council figures and command actors fail closed; cuts-only presentation persists; direct SDL/PipeWire capture proves opt-in behavior. A prior accepted physical-display chart records 195.18 FPS average and zero hitches, but must be repeated on final scene content. Core runtime evidence remains in [`unreal-linux-package-status.json`](docs/production/evidence/unreal-linux-package-status.json) and [`unreal-runtime-acceptance-status.json`](docs/production/evidence/unreal-runtime-acceptance-status.json).

Three original environment/prop increments now pass the same end-to-end production-blockout loop: deterministic Blender source, clean GLB/FBX validation, exact UE 5.8.1 LOD/UV/material/collision-source inspection, compile-clean authored PBR graphs, fail-closed non-authoritative placement, forced cook, clean launch and visible play. The wet-field checkpoint packages exactly three new assets over the command-surface baseline, advances two canonical decisions, completes all three Broken Crossing pulses and returns the exact campaign state. Rejected neural, end-on/gold, pale-plaster, pale-field and over-dark passes are recorded rather than hidden. This proves usable reviewed blockouts—not final art, historical artifacts or surveyed reconstruction. [Wet-field presentation evidence](docs/production/evidence/unreal-wet-field-environment-presentation-status.json) retains the remaining shelter, weather, character, formation, final-performance and human-review gates.
