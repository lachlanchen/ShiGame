...
| [`apps/unreal`](apps/unreal/) | Priority Unreal Engine 5.8 C++ cinematic client: canonical schema-v7/audio/edition loaders, fail-closed source/claim/rights validation, presentation-independent deterministic session, 46-route golden replay suite, fail-closed save/resume, six-layer all-scene progression, bounded five-site wartable plus nine live resource/field/pursuit/method/oath 3D signals with pointer/keyboard/gamepad inspection, scoped historical basis, non-drifting skippable camera focus/response beats, opt-in procedural rain/cues with persisted mix controls and official Linux build wrapper; native compile/play/audio evidence remains open until the official installed build is obtained |
...
Pre-alpha schema-v7/Unreal-priority checkpoint, 2026-08-09. Exact Unreal implementation `ed4dcc6d3e4955f0115f6d98b7ccb83d4fe977c3` passes a detached clean install and complete build: 47 TypeScript/UI tests, 689 successful and 87 capture/scattering condition routes, the regenerated 46-route cross-engine corpus, synchronized Web/Unreal/Unity payloads, Unreal project preflight, accessibility/audio/font/privacy gates and all bundle budgets. Unreal now has a presentation-independent deterministic C++ session, all six resolution layers, exhaustive golden-replay automation source, authoritative save/resume, tamper rejection, protected restart and all-scene progression. Its cinematic command space contains five canonical intelligence sites plus nine authoritative world signals for exact resources, field, pursuit, selected-order method read and carried oath; all are inspectable by pointer, keyboard and gamepad with bounded, non-drifting, skippable camera motion. The last complete visible-browser campaign run remains pinned to `a11e173a58d9290f1368f1596e730a13ccb5e820` with 248 checks across all acts/scenes, desktop/mobile, eleven locales, forced colors, 320px reflow and actual Chrome 400% zoom; the later application-lifetime drawer-focus fix is `58872586da135a221a1741e92bdd1391c0f2b1b3`. Hosted validation `31297150897` and Pages deployment `31297150875` pass at published checkpoint `9cfbd83946cb8c04872a71b4356e1e69fa0f0c9b`; local `HEAD`, `origin/main` and the remote ref matched, and the deployed game returned HTTP 200 before this evidence-note commit. The initial player remains inside the caps at 99.47 KiB JavaScript, 11.94 KiB CSS, 178.95 KiB largest lazy JavaScript, 22.94 MiB fonts and 26.84 MiB deployable output. Unreal is **not yet claimed native-playable or film quality**: the spatial, command-signal and route C++ tests are authored, but official editor acquisition, native compile/automation, PIE interaction, Linux packaging, performance and human review remain red. Unity remains a maintained baseline with its existing license gate. Human listening, assistive-technology, physical-controller, observed-player, Qin-law, geography and translation review also remain open. SHI is a one-year quality-driven production; no source-only engine boundary is allowed to masquerade as a shipped game.
