...
| [`apps/unreal`](apps/unreal/) | Priority Unreal Engine 5.8 C++ cinematic client: canonical schema-v7/audio/edition loaders, fail-closed source/claim/rights validation, inspectable historical-basis panel, presentation-independent deterministic session, 46-route golden replay suite, fail-closed save/resume, keyboard/gamepad command surface, six-layer resolution, all-scene progression, art-directed runtime command space, skippable camera beat, opt-in procedural rain/cues with persisted mix controls and official Linux build wrapper; native compile/play/audio evidence remains open until the official installed build is obtained |
...
Pre-alpha schema-v7/Unreal-priority checkpoint, 2026-08-09. Exact Unreal implementation `e2f523678e8bf849c977bd696bb0703c5d1e41d5` passes a detached fresh-clone install and complete build: 47 TypeScript/UI tests, 689 successful and 87 capture/scattering condition routes, the regenerated 46-route cross-engine corpus, synchronized Web/Unreal/Unity payloads, Unreal project preflight, accessibility/audio/font/privacy gates and all bundle budgets. Unreal now has a presentation-independent deterministic C++ session, all six resolution layers, exhaustive golden-replay automation source, authoritative save/resume, tamper rejection, protected restart, keyboard/gamepad commands, all-scene progression and a cinematic command-space/camera foundation. The last browser-facing implementation remains `a11e173a58d9290f1368f1596e730a13ccb5e820`: its isolated noVNC route passes 248 checks across all acts/scenes, desktop/mobile, eleven locales, forced colors, 320px reflow and actual Chrome 400% zoom. Hosted validation `31292845828` and Pages deployment `31292845859` pass at evidence boundary `4e20b47d3e052411af2d8d40a46efe195b97abbd`; the public game repeats all 248 checks with 250 requests/170 same-origin font slices and zero console errors, cross-origin requests/resources or failed requests. The initial player remains inside the caps at 99.47 KiB JavaScript, 11.94 KiB CSS, 178.95 KiB largest lazy JavaScript, 22.94 MiB fonts and 26.84 MiB deployable output. Unreal is **not yet claimed native-playable or film quality**: the 46 C++ route tests are authored but official editor acquisition, native compile/automation, PIE interaction, Linux packaging, performance and human review remain red. Unity remains a maintained baseline with its existing license gate. Human listening, assistive-technology, physical-controller, observed-player, Qin-law, geography and translation review also remain open. SHI is a one-year quality-driven production; no source-only engine boundary is allowed to masquerade as a shipped game.
