...
Pre-alpha persistent-opposition checkpoint, 2026-08-09. Exact implementation `aa2f4ff1db6f2c9d8e6c8b74bcca6db760bc1129` passes the complete local build/test/content-route/accessibility gates; exact local evidence boundary `be077e1ebcf75d1601b04328ef052fae9a1a6a7b` repeats 186 visible noVNC checks with zero console errors. Chapter I now discloses and records three reachable Qin pursuit postures, exact counterplay and four-layer action/pressure/pursuit/field results; save v4 preserves older decisions under their original rules. The initial player code remains inside the unchanged 100 KiB cap at 99.15 KiB gzip; 9.58 KiB CSS, 178.95 KiB largest lazy JavaScript, 22.94 MiB complete fonts and 26.78 MiB deployable output also pass enforced caps. Hosted validation and Pages deployment pass at `e59a78689239a9dd8e07e02a9b37df5f49014895`; the public build repeats all 186 checks with twenty-seven axe states, nineteen target states, eleven locale/font states, three audio-UI states and zero console, cross-origin-resource or failed-request errors. The prior objective-audio checkpoint remains valid: deterministic reference and public actual-Chrome captures pass their peak/loudness/cue/DC/loop/channel and exact pre-consent-silence gates. A real Unity editor with Linux and Web support is installed, and the expanded Unity runtime/editor/EditMode-test sources—including pursuit rules/preflight/UI/save migration—pass an offline warning-as-error reference-assembly compile. Native import, EditMode execution and player builds still require Unity account licensing; human listening/sensory-load/physical-device/native-audio review, assistive-technology and physical-controller sessions, observed players, and Qin-law/historical-GIS specialists remain open. Those gates remain red rather than hidden. SHI is planned as a one-year quality-driven production, and this repository will not label the goal complete until both clients, research, localization, assets, observed player sessions, and release checks pass.
