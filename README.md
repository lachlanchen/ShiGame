...
| [`apps/unreal`](apps/unreal/) | Priority Unreal Engine 5.8 C++ cinematic client: official UE 5.8.1 native compile and fifteen-suite automation; canonical schema-v7/audio/edition/character loaders; independent 46-route campaign and 76-route command replay; fail-closed durable-first order/restart transactions; complete six-scene progression; bounded wartable/signals, consequence cinema and opt-in sound; plus reviewed original command-weight, fictional earth/wood command-ground, `24 × 24 m` wet-field and open Daze field-shelter production blockouts. The 509-package player visibly advances Act I → Act III, completes the three-pulse engagement beneath the shelter and returns the campaign unchanged. Rain/vegetation, period figures/formations and final lighting remain open. |
...
| [`assets`](assets/) | Reviewed Daze/Broken Crossing lookdev plus AgenticApp/Blender wartable, command-weight, command-surface, wet-field and field-shelter sources, previews, `.blend`, GLB/FBX LODs, rejected-trial records and SHA-256 provenance |
...
Epic's official UE 5.8.1 installed build compiles and links the C++ project and passes exactly fifteen native suites against the byte-identical campaign and engagement payloads. The archived normal-thread Linux Development player visibly completes the three-pulse Broken Crossing exercise without mutating campaign authority, then separately completes, seals and cold-resumes the full four-decision chapter. Real packaged attacks against save writes, restart writes, council figures and command actors fail closed; cuts-only presentation persists; direct SDL/PipeWire capture proves opt-in behavior. A prior accepted physical-display chart records 195.18 FPS average and zero hitches, but must be repeated on final scene content. Core runtime evidence remains in [`unreal-linux-package-status.json`](docs/production/evidence/unreal-linux-package-status.json) and [`unreal-runtime-acceptance-status.json`](docs/production/evidence/unreal-runtime-acceptance-status.json).

Four original environment/prop increments now pass the same end-to-end production-blockout loop: deterministic Blender source, clean GLB/FBX validation, exact UE 5.8.1 LOD/UV/material/collision inspection, compile-clean authored PBR graphs, fail-closed non-authoritative placement, forced cook, clean launch and visible play. The shelter checkpoint packages exactly four new assets over the wet-field baseline, advances two canonical decisions, completes all three Broken Crossing pulses beneath the open structure and returns the exact campaign state. Checker-color, roof-penetration, lashing-axis and LOD-silhouette defects were rejected and corrected before admission. This proves usable reviewed blockouts—not final art, historical artifacts or surveyed reconstruction. [Shelter presentation evidence](docs/production/evidence/unreal-daze-field-shelter-presentation-status.json) retains the remaining rain, vegetation, character, formation, final-performance and human-review gates.
