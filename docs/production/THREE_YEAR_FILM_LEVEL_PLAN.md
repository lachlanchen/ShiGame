# Three-year film-level production charter

Status: active long-range charter · planning horizon August 2026–July 2029 · reviewed 2026-08-10

This charter turns SHI's long-term ambition into staged, testable production work. It does not declare the current blockout film quality, promise that every desired tool or classical text will ship, or replace the detailed one-year [`ROADMAP.md`](ROADMAP.md). The roadmap is the synchronized near-term schedule for this charter's Year 1 scope; neither document may silently promise a broader release or campaign horizon than the other.
...
As of 2026-08-10, the Web campaign and packaged Unreal route are genuinely interactive, and shared deterministic rules, source ledgers, localization infrastructure and bounded accessibility checks exist. Unreal passes twenty native `SHI.` suites. The accepted council facial v2 package proves five shared-skeleton engineering figures, an exact 21-control silent-intent contract and corrected morph-capable materials.
...
5. Each production increment must be narrow enough to validate in source, engine, package and visible play before the same pipeline scales.
6. Open-source or AI tooling is admitted only for a demonstrated bottleneck, pinned/versioned where possible, license-reviewed, locally inspectable and replaceable. Tool popularity is not a quality gate.

## Three-year sequence

Dates are planning ranges, not promises. A year advances only when its exit evidence passes; unfinished gates roll forward instead of being relabeled complete.

### Year 1 — foundation and two production vertical slices
...
#### Months 5–8: first final-camera Daze slice
...
#### Months 9–12: second slice and pipeline repeatability
...
#### Months 13–18: chapter pods and systemic depth
...
#### Months 19–24: cinematic and content production
...
#### Months 31–34: alpha, beta and optimization
...
#### Months 35–36: release candidate
...
Tools accelerate candidate creation and inspection; they do not own story truth or approve their own output.
...
Any open-source repository, model or hosted service is evaluated before installation against license, provenance, security, maintenance, hardware/storage cost, editability and the specific bottleneck it removes. A downloaded or generated artifact stays quarantined outside shipping paths until its admission record is green. Existing verified SDKs, weights and profiles are reused instead of duplicated.
...
- **Weekly:** identify one player-facing problem, implement one bounded integration, run automated and visible checks, triage defects and update truth-bearing docs.
...
This charter is deliberately demanding and revisable. The goal is not to spend three years filling the repository; it is to make every year leave behind a more complete, enjoyable and trustworthy game.
