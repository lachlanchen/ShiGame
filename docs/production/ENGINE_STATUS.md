...
Last checked: 2026-08-10, Asia/Hong_Kong.
...
- Unreal project-file generation, `SHIEditor` compilation/linking with bundled Clang 20.1.8, and the exact sixteen-suite `SHI.` native automation run; all sixteen suites pass with exit code 0, including command-weight, command-surface, wet-field, Daze-shelter and Daze-rain presentation, 76-route Broken Crossing parity and bounded six-piece spatial contracts.
...
- Command-space blockouts: the exact command weight, fictional earth/wood surface, `24 × 24 m` wet field, open Daze field shelter and deterministic rain all pass source, LOD/UV/material, Unreal admission, compile-clean graph, runtime-authority and visible presentation checks. The latest clean package has 513 packages, advances Act I → Act III, completes all three Broken Crossing pulses in rain beneath the dry shelter and returns the byte-identical campaign save. Primitive figures, floating light, BasicShape signals, vegetation, formations and final lighting/weather response remain rejected as final art.
...
The official installed build now compiles the committed Unreal runtime and executes the project automation. Project generation uses Epic's Linux batch script; targets use Unreal 5.8's `BuildSettingsVersion.V7`; `SHIEditor` compiles and links with bundled Clang 20.1.8. The exact `SHI.` namespace discovers sixteen—not similarly named engine—suites. Audio, all 46 campaign replays, immutable order transaction, save integrity, schema horizon, canonical council staging, command-weight/surface/wet-field/shelter/rain presentation, consequence grammar, nine campaign signals, 76-route Broken Crossing parity with six bounded spatial tallies, historical source/claim closure and wartable spatial intelligence all pass with exit code 0. Native execution also exposed and drove correction of a Keeper self-speaker defect in `broken-crossing`; regenerated fixtures bind campaign SHA-256 `a82ab2f28809b46fb780074cce8acb96a7eb0b7cd608868f03b1888b64acf1f0`. Tests use `-nowrite`; intentional tracked configuration force-cooks each admitted authored-asset path.
...
1. Repeat the engagement campaign-byte guard after deliberate in-memory drift when a bounded runtime hook exists, preserving the last accepted save.
2. Connect a physical Xbox/PlayStation-layout controller and complete the remaining council/site/signal/evidence route plus natural consequences. Review exact deltas/text, speaker provenance, highlights, lenses, isolation, collision, silhouettes, eyelines, lighting and comfort.
3. Perform human listening review of the measured native audio path for rapid cues, ambience fatigue, stop/resume and device behavior; numeric capture alone is insufficient.
4. Extend the accepted command-surface/wet-field/shelter/rain blockouts with vegetation, period-reviewed figures/interaction hands, formations and lighting/weather response, without changing deterministic command boundaries; then repeat physical visual, performance and observed-player gates.
