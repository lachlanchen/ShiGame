...
- Unreal project-file generation, `SHIEditor` compilation/linking with bundled Clang 20.1.8, and the exact twelve-suite `SHI.` native automation run; all twelve suites pass with exit code 0, including the command-weight presentation, 76-route Broken Crossing parity and bounded six-piece spatial contracts.
...
- Command-weight runtime blockout: exact mesh scale/placement/contact, noninteractive authority, 10-node stone and 14-node bronze graphs, 44° council composition, development-only front/back review, 499-package rebuild/headless smoke and visible Act-I-to-Act-II transaction progression all pass. The white BasicShape command surface and proxy figures remain rejected as final art.
...
4. Replace the noisy white BasicShape command surface first, then proxy figures/interaction hands, terrain, formations and lighting, without changing the accepted deterministic command boundaries; repeat physical visual, performance and observed-player gates.
