...
As of 2026-08-09, the expanded runtime, editor and EditMode-test sources—including wartable marker picking, inspected-site state, localized intelligence UI, shared audio contract loading/validation, zero-mean deterministic rain, shared cue envelopes, peak/loudness/DC/loop/channel quality bounds, localized mixer UI, three Qin pursuit postures, exact precommit modifiers/counterplay, four-layer resolution, save-v4 identity checks, legacy replay and site closure validation—compile with warnings treated as errors against the installed Unity/Newtonsoft/NUnit reference assemblies. Unity preflight independently reconstructs the raw rain and rejects excessive DC offset or a loop-boundary transient outside the shared limit; EditMode source tests also calculate cue peaks and pursuit/save behavior. The deterministic FFmpeg reference and actual-Chrome measurements prove the common authored contract and web path, not Unity's native mixer/output. This remains an offline source-type check, not an Editor import, audio-device review or test run, and does not change the red gate above.
