...
- Unreal project-file generation, `SHIEditor` compilation/linking with bundled Clang 20.1.8, and the exact fourteen-suite `SHI.` native automation run; all fourteen suites pass with exit code 0, including command-weight, command-surface and wet-field presentation, 76-route Broken Crossing parity and bounded six-piece spatial contracts.
...
- Command-space blockouts: the exact command weight, fictional earth/wood surface and `24 × 24 m` wet field all pass source, LOD/UV/material, Unreal admission, compile-clean graph, runtime-authority and visible presentation checks. The latest clean package has 505 packages, advances Act I → Act III, completes all three Broken Crossing pulses and returns the campaign unchanged. Primitive figures, floating light, BasicShape signals, shelter/rain/vegetation, formations and final lighting remain rejected as final art.
...
The official installed build now compiles the committed Unreal runtime and executes the project automation. Project generation uses Epic's Linux batch script; targets use Unreal 5.8's `BuildSettingsVersion.V7`; `SHIEditor` compiles and links with bundled Clang 20.1.8. The exact `SHI.` namespace discovers fourteen—not similarly named engine—suites. Audio, all 46 campaign replays, immutable order transaction, save integrity, schema horizon, canonical council staging, command-weight/surface/wet-field presentation, consequence grammar, nine campaign signals, 76-route Broken Crossing parity with six bounded spatial tallies, historical source/claim closure and wartable spatial intelligence all pass with exit code 0. Native execution also exposed and drove correction of a Keeper self-speaker defect in `broken-crossing`; regenerated fixtures bind campaign SHA-256 `a82ab2f28809b46fb780074cce8acb96a7eb0b7cd608868f03b1888b64acf1f0`. Tests use `-nowrite`; the only intentional tracked configuration delta force-cooks the admitted wet-field path.
...
4. Extend the accepted command-surface/wet-field blockouts with shelter, bounded rain, vegetation, period-reviewed figures/interaction hands, formations and lighting, without changing deterministic command boundaries; then repeat physical visual, performance and observed-player gates.
