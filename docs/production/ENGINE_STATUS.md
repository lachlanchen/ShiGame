...
- Unreal project-file generation, `SHIEditor` compilation/linking with bundled Clang 20.1.8, and the exact fifteen-suite `SHI.` native automation run; all fifteen suites pass with exit code 0, including command-weight, command-surface, wet-field and Daze-shelter presentation, 76-route Broken Crossing parity and bounded six-piece spatial contracts.
...
- Command-space blockouts: the exact command weight, fictional earth/wood surface, `24 × 24 m` wet field and open Daze field shelter all pass source, LOD/UV/material, Unreal admission, compile-clean graph, runtime-authority and visible presentation checks. The latest clean package has 509 packages, advances Act I → Act III, completes all three Broken Crossing pulses beneath the shelter and returns the campaign unchanged. Primitive figures, floating light, BasicShape signals, rain/vegetation, formations and final lighting remain rejected as final art.
...
The next admitted block closes the empty-sky shelter defect without claiming archaeology. Deterministic Blender sources and clean GLB/FBX round trips preserve an `8.40 × 6.73 × 3.55 m` four-post, open-sided wood/reed/cord shelter with `3,948 / 460` triangles, dual UVs and zero collision. Unreal authors compile-clean `8 / 11 / 8`-node PBR graphs, keeps the identity-root structure noninteractive/non-navigable, and rejects unsupported attestation or premature final-art state. The clean player cooks and mounts `509` packages—exactly four beyond the wet-field baseline. Visible pointer play advances Act I → Act III, reaches **Orderly crossing**, and returns to the exact `41 / 68 / 17 / 80 / 63` campaign state. This is accepted only as a fictional practical production blockout; rain, vegetation, period figures/formations, final sky/lighting and human review remain red.
...
4. Extend the accepted command-surface/wet-field/shelter blockouts with bounded rain, vegetation, period-reviewed figures/interaction hands, formations and lighting, without changing deterministic command boundaries; then repeat physical visual, performance and observed-player gates.
