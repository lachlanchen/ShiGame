...
4. Run the complete visible site/signal click and cycle → inspect all nine states → site evidence → current ground → select and observe method-read change → issue → six/seven consequence beats → act/time progression route. Capture natural completion and mid-beat skip; confirm exact deltas/text, one highlighted target, input isolation, authoritative camera return and unchanged history while also reviewing lighting/UI/audio, silence before opt-in, mixer persistence, rapid cues and stop/resume.
