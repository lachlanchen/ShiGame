...
Three real packaged fault injections are now green. An unwritable SaveGames directory/file creates no temp file and shows **Order not issued · Autosave failed** without changing the exact save. Development-console removal of both `AShiCouncilFigure` actors or all generic command/site actors makes order preflight name the unavailable council `speaker` or `resource-grain` signal and hold chronicle/world before persistence. Each injected run preserves save SHA-256 `e9cbb4c857005fc04ac3dfe63b735296566655e0675e12c8045e8ccede2f42e8`; restart restores the actors. Failed-restart injection remains open.

Cuts-only runtime acceptance is green for the observed order. `V` changes the visible state to **Reduced motion · Cuts only**; the durable order then exposes the exact seven-beat consequence, supports presentation-only skip and hands control to the next canonical council. Restart resumes Turn 4 plus the cuts-only preference. Native SDL output is also measured: direct PipeWire linkage captures exact digital silence for 4.01 seconds before opt-in and 3.97 seconds after disable, while the 16.13-second enabled stream measures −39.7 LUFS, −27.7 dBFS peak, 3.6 LU LRA, zero DC and exact channel parity. This is engineering evidence, not human listening approval.

Stable editor PIE is still red. The official editor rendered the real 33-actor command space and entered PIE, then Vulkan reported `AcquireNextImage() failed due to the outdated swapchain` and crashed in NVIDIA userspace (`libnvidia-glcore.so.595.84`). Physical-controller feel is also red because the workstation currently exposes no joystick or USB game controller. Full natural-motion comfort/lens review, failed restart, human native-audio listening, reviewed final assets, film quality and observed human playability remain open rather than inferred from package stability.
...
1. Inject a failed restart and repeat the engagement campaign-byte guard after deliberate drift, preserving the last accepted save.
