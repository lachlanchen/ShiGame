...
- Unreal project-file generation, `SHIEditor` compilation/linking with bundled Clang 20.1.8, and the exact ten-suite `SHI.` native automation run; all ten suites pass with exit code 0.
...
### Unreal visible-play and package proof

The official installed build now compiles the committed Unreal runtime and executes the project automation. Project generation uses Epic's Linux batch script; targets use Unreal 5.8's `BuildSettingsVersion.V7`; `SHIEditor` compiles and links with bundled Clang 20.1.8. The exact `SHI.` namespace discovers ten—not similarly named engine—suites. Audio, all 46 campaign replays, immutable order transaction, save integrity, schema horizon, canonical council staging, consequence grammar, nine live signals, historical source/claim closure and wartable spatial intelligence all pass with exit code 0. Native execution also exposed and drove correction of a Keeper self-speaker defect in `broken-crossing`; regenerated fixtures now bind campaign SHA-256 `a82ab2f28809b46fb780074cce8acb96a7eb0b7cd608868f03b1888b64acf1f0`. Tests use `-nowrite`, and tracked Unreal configuration remains byte-identical across the run.

This proof does not yet include the shared three-pulse engagement in C++, visible PIE interaction, real file-write/actor/figure fault injection, camera/handoff/lens/reduced-motion observation, audible native device output, reviewed final assets, film quality or a packaged Linux player. Those claims remain red rather than being inferred from compilation or headless automation.
...
1. Implement the shared Broken Crossing engagement in presentation-independent C++, prove exact exhaustive route counts, determinism and hostile replay rejection, and keep it non-authoritative until parity is accepted.
2. Open the official editor through the localhost-only `:121` noVNC desktop and verify Vulkan/PIE presentation without allowing generated project preferences to pollute tracked configuration.
3. Run the complete visible opening speaker/Keeper shot → figure click plus `D`/R3/Slate return → site/signal click and cycle → inspect all nine states → site evidence → council/current ground → select → three command pulses → verified campaign commit → consequence beats → exact next-speaker handoff. Inject one missing focus actor, one missing council figure and one candidate-save write failure and prove each holds the order/history/world/cast; repeat for failed restart.
4. Capture natural completion and mid-beat skip in restrained mode, then repeat inspection and one consequence/handoff in persisted cuts-only mode. Review exact deltas/text, named speaker/provenance, highlight, cut/ease thresholds, lenses, input isolation, controller feel, collision, silhouettes, eyelines, lighting/UI/audio, silence before opt-in, mixer persistence, rapid cues and stop/resume.
