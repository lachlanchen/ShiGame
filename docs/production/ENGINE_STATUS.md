...
2. Set `SHI_UNREAL_ROOT`, then run `./scripts/unreal-pipeline.sh projectfiles`, `build`, and `test`; spatial-wartable, live-command-signal, consequence-cinema, source/claim closure, 46-route replay and procedural-audio suites are the first native behavior gates.
3. Open the project through `./scripts/unreal-pipeline.sh editor`; fix API/import issues under native compiler evidence.
4. Run the complete visible site/signal click and cycle → inspect all nine states → site evidence → current ground → select and observe method-read change → issue → six/seven consequence beats → act/time progression route. Capture natural completion and mid-beat skip in restrained mode, then repeat inspection and one consequence in persisted cuts-only mode. Confirm exact deltas/text, one highlighted target, cut/ease threshold behavior, semantic lenses, input isolation, authoritative 50° camera return, unchanged history and no accessibility-to-audio side effect while also reviewing lighting/UI/audio, silence before opt-in, mixer persistence, rapid cues and stop/resume.
