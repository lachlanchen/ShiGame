...
Unreal reprioritization status on 2026-08-09: a real Unreal 5.8 C++ project now stages the byte-identical schema-v7 campaign/audio payload, validates its three-act horizon, resolves the six gameplay layers, advances all six authored scenes and renders a side-by-side 3D command/UI composition. Its source-authored consequence grammar binds exact resolved order/oath/pressure/pursuit/method/field/position records to live world actors in a deterministic 3.52–4.08-second sequence, blocks unrelated commands and skips back to authoritative ground; every golden-route turn must build a valid plan. Static and detached clean-build gates pass. Official editor acquisition, native compile/automation, visible PIE motion/input review, reviewed art/audio, Linux packaging and observed player evidence remain open; none is represented as complete.
