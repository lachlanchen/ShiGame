...
Martial-command status on 2026-08-09: the first versioned broken-crossing engagement candidate, pure shared resolver and Web command-board reference are implemented. Its three plans, two conditions, three pulses, nine commands and four outcomes close to the existing campaign node and registered historical claims without activating any unreviewed martial source. Exhaustive traversal proves 76 deterministic routes, 47 success/costly-success routes, no deadlock, two-or-more viable plans per condition, every command/outcome reachable and hostile replay rejection. The Web path binds the current selected plan/seeded condition, filters legal orders, separates field answers from player effects, exposes six local states, preserves the campaign save byte-for-byte and is visibly proven with pointer/keyboard/standard-gamepad input on desktop and mobile. It remains explicitly non-authoritative until native Unreal implements and independently replays the same record; spatial terrain/formations/signals, film-language review and campaign migration therefore remain open.
...
Unreal reprioritization status on 2026-08-09: Epic's official UE `5.8.1` installed build (changelist `56057345`) now generates the project, compiles/links `SHIEditor` and passes exactly ten native `SHI.` suites. Those suites execute all 46 campaign routes plus save, transaction, source/claim, audio, wartable, signal, council and consequence contracts against campaign hash `a82ab2f28809`; the native run found and drove correction of a Keeper self-speaker content defect. The real C++ project stages the byte-identical schema-v7 campaign/audio payload, validates its three-act horizon plus canonical character/speaker closure, resolves the six gameplay layers and advances all six authored scenes. Every issued campaign order resolves on a copy, independently revalidates its rule/world/council/camera sentence and writes before active memory advances. The next feature gate is the independent native implementation of the three-pulse Broken Crossing engagement and its bounded 3D interaction. Visible PIE success and injected failures, handoff/motion/lens/input/accessibility review, native audio-device review, reviewed final character/environment assets, Linux packaging and observed player evidence remain open; none is represented as complete.
