...
Unreal reprioritization status on 2026-08-10: Epic's official UE `5.8.1` installed build (changelist `56057345`) generates the project, compiles/links the runtime plus editor-only animation module and passes exactly nineteen native `SHI.` suites. Those suites execute all 46 campaign routes plus the 76-route Broken Crossing exercise and its save, transaction, source/claim, audio, wartable, spatial-signal, council, character, body-performance, command-weight, command-surface, wet-field, shelter, rain, vegetation and consequence contracts. The latest Linux development package mounts 538 packages. Five corrected identity-Root shared-skeleton characters and two 4-second/30 fps/121-sample body-performance loops now have source/interchange, exact Unreal import, hostile-drift automation, package/smoke and visible-play receipts. Four earlier animation packages were rejected until a 10000x Skeleton Root defect was isolated and corrected; the accepted full body remains intact while a real packaged input route advances Act I → Act II and autosaves. This is still a body-performance blockout: facial/eye/speech, interaction hands, historically reviewed etiquette/formations, physical controller, final-scene performance, natural-motion/human-audio review and observed-player evidence remain open. A physical 60 Hz FPS chart records 195.18 FPS average on the prior accepted scene and zero hitches; timing must be repeated on final content. Editor PIE remains red after an NVIDIA Vulkan outdated-swapchain crash.
