...
Unreal reprioritization status on 2026-08-09: a real Unreal 5.8 C++ project now stages the byte-identical schema-v7 campaign/audio payload, validates its three-act horizon, resolves the six gameplay layers, advances all six authored scenes and renders a side-by-side 3D command/UI composition. Every issued order now resolves on a copied session, builds and independently revalidates its exact rule record, next briefing, nine live signals and complete camera sentence, proves runtime actor closure, and writes the candidate save before active memory advances; two-step restart uses the same durable-first discipline. All 46 golden-route turns cross this boundary and hostile source tests attack hidden decisions plus rule/world/selection/cinema drift. The consequence grammar binds exact order/oath/pressure/pursuit/method/field/position records to a deterministic 3.52–4.08-second sequence with bounded motion, semantic 40°–58° lenses, command isolation, persisted cuts-only reduced motion and authoritative skip return. Static and detached clean-build gates pass. Official editor acquisition, native compile/automation, visible PIE success/write-failure/actor-failure and motion/lens/input/accessibility review, reviewed art/audio, Linux packaging and observed player evidence remain open; none is represented as complete.
