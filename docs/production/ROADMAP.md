...
Martial-command status on 2026-08-09: the first versioned broken-crossing engagement candidate and pure shared resolver are authored. Its three plans, two conditions, three pulses, nine commands and four outcomes close to the existing campaign node and registered historical claims without activating any unreviewed martial source. Exhaustive traversal proves 76 deterministic routes, 47 success/costly-success routes, no deadlock, two-or-more viable plans per condition, every command/outcome reachable and hostile replay rejection. It remains explicitly non-authoritative until Web and native Unreal implement and replay the same contract.
...
Unreal reprioritization status on 2026-08-09: a real Unreal 5.8 C++ project now stages the byte-identical schema-v7 campaign/audio payload, validates its three-act horizon plus canonical character/speaker closure, resolves the six gameplay layers, advances all six authored scenes and renders a side-by-side 3D command/UI composition. Every issued order now resolves on a copied session, builds and independently revalidates its exact rule record, next briefing, nine live signals, next speaker/Keeper council blocking/disclosure and complete camera sentence, proves runtime actor/figure closure, and writes the candidate save before active memory advances; two-step restart uses the same durable-first discipline. All 46 golden-route turns cross this boundary and hostile source tests attack hidden decisions plus rule/world/selection/council/cinema drift. The consequence grammar binds exact order/oath/pressure/pursuit/method/field/position records to a deterministic 3.52–4.08-second sequence with bounded motion, semantic 40°–58° lenses, command isolation and persisted cuts-only reduced motion, then hands control to the correct named speaker. Two persistent body/head/mantle proxies establish click, silhouette, eyeline, camera and asset-swap contracts without claiming final character art. Static and local complete-build gates pass at implementation `5764a2dc7071ea2355214ae1aec6f9922e1a2a65`. Official editor acquisition, native compile/automation, visible PIE success/write-failure/actor-failure/figure-failure and handoff/motion/lens/input/accessibility review, reviewed final character/environment/audio assets, Linux packaging and observed player evidence remain open; none is represented as complete.
