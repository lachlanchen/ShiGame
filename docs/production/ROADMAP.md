...
Martial-command status on 2026-08-09: the first versioned Broken Crossing engagement, pure shared resolver, Web command-board reference and independent native Unreal implementation are complete at the non-authoritative boundary. Its three plans, two conditions, three pulses, nine commands and four outcomes close to the existing campaign node and registered historical claims without activating any unreviewed martial source. Web and official Unreal independently match 76 deterministic routes, 47 success/costly-success routes, no deadlock, two-or-more viable plans per condition, every command/outcome reachable and hostile replay rejection. Unreal adds six bounded shape/order/height/stencil tallies, candidate-copy pulse commits and an exact campaign-save guard; a visible noVNC route reaches an outcome and preserves the disk save hash before/during/after the exercise. Campaign migration, reviewed terrain/formations, film-language polish and observed-player balance remain open.
...
Unreal reprioritization status on 2026-08-10: Epic's official UE `5.8.1` installed build (changelist `56057345`) generates the project, compiles/links `SHIEditor` and passes exactly twelve native `SHI.` suites. Those suites execute all 46 campaign routes plus the 76-route Broken Crossing exercise and its save, transaction, source/claim, audio, wartable, spatial-signal, council, command-weight presentation and consequence contracts against campaign hash `a82ab2f28809`; native runs found and drove correction of a Keeper self-speaker content defect, a hidden-five-of-six command composition defect, and a face-only council focus that lost the physical decision plane. The real C++ project stages the byte-identical schema-v7 campaign/audio/engagement payload, validates its three-act horizon plus canonical character/speaker closure, resolves the six campaign layers and independently resolves all three tactical pulses without mutating campaign authority. The official Linux development package passes compile/cook/stage/archive, and its normal-thread player visibly completes Broken Crossing while preserving the exact campaign save, then seals and cold-replays the full four-decision chapter. The first authored command-weight production blockout additionally passes exact Unreal material graphs, contact/clearance/44° lens placement, front/back packaged review, 499-package smoke and visible Act-II progression; a first end-on/polished-gold pass was rejected. A physical 60 Hz FPS chart records 195.18 FPS average and zero hitches; noVNC's lower rate is isolated to Xvfb/capture presentation. Packaged unwritable-save, failed-restart, missing-council and missing-command-signal attacks hold state; cuts-only consequence/skip/restart persistence and direct native audio capture pass. Editor PIE remains red after an NVIDIA Vulkan outdated-swapchain crash; the white BasicShape surface, proxy figures/terrain/formations, physical controller, natural-motion/human-audio review and observed-player evidence remain open.

## Weekly operating cadence

- Monday: player problem, evidence and scope review.
- Tuesday–Wednesday: one integrated gameplay increment.
- Thursday: historical/localization/asset review and automated validation.
- Friday: visible playtest, defect triage, progress log and clean build.
- Monthly: milestone kill/continue decision based on player evidence, not sunk cost.
...
- Do not call a milestone complete when its build or review gate is blocked.
