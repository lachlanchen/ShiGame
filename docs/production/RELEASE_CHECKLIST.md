...
Six authored Unreal blocks now close a bounded runtime subset. `shi-command-weight-v1`, `shi-command-surface-v1`, `shi-wet-field-environment-v1`, `shi-daze-field-shelter-v1`, `shi-daze-rain-vfx-v1` and `shi-daze-wet-field-vegetation-v1` pass deterministic source/interchange validation, explicit LOD/UV/material/collision contracts, UE 5.8.1 admission, compile-clean blockout graphs, forced cook, clean archived-player launch and visible story/engagement use. The vegetation package cooks 516 packages—exactly the accepted 513 rain baseline plus two meshes and one material—and visibly completes Broken Crossing before returning the byte-identical canonical campaign save. The project-wide 3D box stays open because formations, period-reviewed characters/interaction hands, final materials/sky/lighting/weather response, physical final-scene performance/accessibility and human art/cinematic/historical/botanical review remain incomplete. See [`unreal-daze-wet-field-vegetation-presentation-status.json`](evidence/unreal-daze-wet-field-vegetation-presentation-status.json) and [`unreal-daze-wet-field-vegetation-import-status.json`](evidence/unreal-daze-wet-field-vegetation-import-status.json).

The bounded council facial lane now passes exact source/interchange admission, twenty native `SHI.` suites and packaged visual engineering review. Facial v2 cooks and mounts 559 packages—five morph-bearing meshes, fifteen materials and one CC0 eye texture over the accepted 538-package body-performance baseline—and preserves the exact 21-control deterministic silent-intent contract on every mesh. Packaged v1 was rejected after missing `MorphTargets` usage on `SkinClay` and `EyeBrown` caused **Default Material** fallback; corrected v2 saves all fifteen materials for `SkeletalMesh` and exactly those two for `MorphTargets`. Watched normal-speaker, reduced-speaker and normal-Keeper views pass. Facial v2 did not rerun story progression or a separate headless smoke; prior v5 remains the latest proof of those paths. The art gate stays open for interaction hands, mouth interior, voice/lip sync, close framing, visible fail-closed fallback, final art/acting and human historical/cultural/accessibility review. See [`unreal-daze-council-facial-performance-presentation-status.json`](evidence/unreal-daze-council-facial-performance-presentation-status.json).
