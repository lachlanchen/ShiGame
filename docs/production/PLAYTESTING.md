...
2026-08-09 readable Qin method-read checkpoint: exact implementation `d7afcd6bd7e838a64397f500e919af112221dcc8` passes `npm ci` and the complete build from a fresh clone, including schema, exhaustive-route, type, unit/UI, static-accessibility and bundle-budget gates. The dedicated visible localhost run against that exact commit passes all 202 checks with twenty-eight axe states, twenty target states, eleven locale/font states and three audio-UI states, with zero console errors, cross-origin requests/resources or non-cancelled failures. The browser proves neutral opening memory, two `witnessed-compact` observations preparing `witness-chain`, exact +3 Exposure forecast and hit, a visible `distributed-cover` miss alternative, isolated five-layer deltas and save-v5 identity. The first run rejected a decision card that no longer fit the 228-CSS-pixel actual-400%-zoom viewport; the corrected card remains wholly focusable and internally scrollable. The next run exposed an offscreen coordinate assumption in the test action, which now visibly scrolls the intended card before pointer commitment. Full rerun evidence—including `web-31-method-read-witness-chain.png`, `web-32-method-read-hit.png` and the corrected `web-25-browser-zoom-400-decisions.png`—is visually approved. Public deployment and replay of this checkpoint remain the next gate; native Unity and human review gates remain open.
