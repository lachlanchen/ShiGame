...
| `shi-daze-rain-vfx-v1` | Approved runtime-presented Unreal production VFX blockout; not final environment | Default-material fallback, skipped material-expression deletion and a mutating inspect-only import were rejected. The accepted fixed pools use 384 crossed-ribbon streaks and 72 exposed-ground rings, exact two-level LODs, dual UVs, vertex alpha, zero collision and two compile-clean texture-free translucent materials. The 513-package player keeps the shelter interior dry, visibly completes two campaign decisions plus Broken Crossing and returns the exact save unchanged. It is dramatic reconstruction, not evidence of exact Daze weather in 209 BCE. |
| `shi-daze-wet-field-vegetation-v1` | Approved runtime-presented Unreal production vegetation blockout; not final environment | The first compiled archive was rejected because string-only references did not cook the new assets. The accepted force-cooked layer uses 42 sparse stalk clumps and 64 low tufts, exact two-level LODs, dual UVs, root-to-tip vertex alpha, zero collision and one compile-clean texture-free 15-node GPU-wind material. Exact shelter/command and approach exclusions remain clear; the 516-package player visibly completes the full story/engagement route and returns the exact save unchanged. The forms are generic—not an exact botanical, crop, seasonal or surveyed Daze reconstruction. |
...
The same bounded pipeline now admits the fictional command surface, wet-field environment, open field shelter and deterministic rain through [`COMMAND_SURFACE_BRIEF.md`](COMMAND_SURFACE_BRIEF.md), [`FIELD_ENVIRONMENT_BRIEF.md`](FIELD_ENVIRONMENT_BRIEF.md), [`DAZE_FIELD_SHELTER_BRIEF.md`](DAZE_FIELD_SHELTER_BRIEF.md) and [`DAZE_RAIN_VFX_BRIEF.md`](DAZE_RAIN_VFX_BRIEF.md). Their editable sources, LODs, engine graphs, exact package deltas, rejected visual passes and complete visible story/engagement receipts are hash-bound under `assets/provenance/` and `docs/production/evidence/`. None waives vegetation, period-character, formation, sky, final-lighting/weather response, performance/accessibility or human historical/cinematic/art gates.
