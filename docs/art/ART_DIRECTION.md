...
The same bounded pipeline now admits the fictional command surface and wet-field environment through [`COMMAND_SURFACE_BRIEF.md`](COMMAND_SURFACE_BRIEF.md) and [`FIELD_ENVIRONMENT_BRIEF.md`](FIELD_ENVIRONMENT_BRIEF.md). Their editable sources, LODs, engine graphs, exact package deltas, rejected visual passes and complete visible story/engagement receipts are hash-bound under `assets/provenance/` and `docs/production/evidence/`. Neither blockout waives shelter, rain, vegetation, period-character, formation, sky, lighting, performance/accessibility or human historical/cinematic/art gates.
