...
- C++ parses the canonical schema-v7 payload plus the registered edition metadata, validates act/time/site transitions and source → claim → scene/site closure, and resolves order → oath → pressure → pursuit → prepared method read → deterministic field condition.
...
- `FShiOrderTransactionModel` resolves on a copied session, builds the post-order selection/signals/cinema, independently recomputes and exactly compares every gameplay and presentation field, and leaves the active session immutable until runtime actor and durable-save preflight pass. Every golden-route turn uses this boundary.
...
- The runtime builds a Daze command space with directional moonlight, motivated fire light, fog, ground and the five-site wartable. Within the prepared transaction, `FShiCinematicBeatModel` derives a stable order → optional oath → pressure → pursuit → method read → field → position sequence from actual intermediate records and exact final resources, then GameMode proves every focus has a live actor before durable commit. The first/unknown shot and any move beyond 100 Unreal units or 6° cut; only bounded neighbors ease through translation, quaternion rotation and a fixed semantic 40°–58° FOV grammar. The six-or-seven-beat sequence lasts 3.52–4.08 seconds, blocks unrelated commands, and `Space`/`Escape`/Gamepad B returns immediately to the authoritative current site and 50° inspection lens without campaign mutation or cumulative drift. A separately persisted `V`/Gamepad Menu/Slate reduced-motion setting makes all inspection and consequence travel cuts-only while retaining reading time and lens meaning. Engine basic-shape packages are explicitly cooked and selected-marker stencils are enabled.
...
- Static repository validation is green. Unreal automation source covers immutable order preflight, exact prepared-session replay, hidden-decision/rule/world/selection/camera drift attacks and atomic failure across every turn of all 46 routes; bounded wartable projection/camera/status/cycling/overlap rejection; exact live-signal values/order/anchoring/camera/cycling, active-oath and captured-terminal states; consequence order/timing/world closure/terminal failure, deterministic cut/ease authorship and semantic lens identity; source/claim/rights closure and hostile URL drift; every intermediate gameplay layer; save round-trip/tamper rejection; and procedural-audio contracts. Native target compilation, automation execution, PIE order/restart write and actor failure injection, site/signal/evidence/consequence interaction, audio-device capture/listening, asset import, Linux packaging and human cinematic/playability review remain red until an official Epic installed build is available.
