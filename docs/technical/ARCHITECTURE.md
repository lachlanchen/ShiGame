...
- `FShiCommandWeightPresentationModel` admits one original fictional physical-decision object without giving it gameplay authority. It pins the exact mesh/bounds/scale, `20°` presentation yaw, command-surface contact, table-edge and site/signal clearance, minimum council-frame readability and the canonical 44° lens. Runtime spawns it collision-free and noninteractive, hides it during the non-authoritative engagement and exposes only development-build 28° front/back review cameras. Its two isolated authored production-blockout materials are maintained by a read-only-by-default Unreal Python tool that validates exact graphs, PBR outputs, parameters and compile state.
...
- Static repository validation is green. Official UE 5.8.1 project generation and `SHIEditor` compile/link pass, and an exact `SHI.` filter discovers and passes twelve native automation suites. Coverage includes immutable order preflight and all 46 campaign routes; exact 76-route Broken Crossing parity, six-piece spatial rebuilds and hostile replay/authority attacks; canonical speaker/Keeper occupancy and evidence classification; command-weight contact/clearance/lens/negative-case presentation; wartable bounds; exact live campaign signals; consequence order/timing/world/motion/lens closure; source/claim/rights closure; save replay/tamper rejection; and procedural audio. The official Linux package passes compile/cook/stage/archive, and its normal-thread player visibly completes three tactical pulses while preserving the packaged campaign save hash. A real-display chart records 195.18 FPS average and zero hitches; Xvfb/noVNC is separately presentation-limited. Packaged write/council/signal failures hold before commit, cuts-only consequence/skip/restart persistence passes and direct SDL/PipeWire capture proves the audio opt-in/off boundary. The 499-package command-weight council build also smoke-tests cleanly and visibly advances the normal story to Act II. Stable editor PIE remains red after an NVIDIA Vulkan outdated-swapchain userspace crash; failed restart, physical controller, human accessibility/audio review, final assets and human cinematic/playability review remain open.
...
`scripts/validate-web-build.mjs` measures the built bytes with one deterministic gzip implementation and fails the build on regression. The current build is 99.86 KiB initial JavaScript, 11.94 KiB initial CSS, 1.99 KiB deliberate decision inspector, 2.32 KiB strategic map, 5.82 KiB commitment presentation, 6.58 KiB opponent presentation, 7.41 KiB evidence/claim UI, 4.33 KiB field guide, 4.09 KiB detailed audio settings, 1.24 KiB audio engine, 0.93 KiB font loader and 178.95 KiB Three.js, all gzip. The complete 565-file Unicode-range font artifact is 22.94 MiB and the source-map-free deployable site is 26.88 MiB. That artifact size is not an initial player transfer: browser requests are limited by active locale, visible glyph ranges and invoked feature boundaries. Axe-core is development-only and does not enter the player bundle.
