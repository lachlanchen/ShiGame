...
- `scripts/validate-content.mjs` checks identifiers, links, translations, effects, reachability, cycles, and conclusions.
...
Choice resolution contains no random number generator. A campaign id, state, and choice id produce the same next state. Future uncertainty mechanics must use a recorded seed and expose the distribution before commitment.
...
- Browser state is namespaced in `localStorage` and versioned.
...
- PlayerPrefs stores the pre-alpha state; production saves will move to signed/versioned files with migrations.
...
The current web build is 85 KiB gzip initial JavaScript and 185 KiB gzip lazy Three.js.
