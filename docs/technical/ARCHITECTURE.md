...
- Static repository validation is green. Unreal automation source covers immutable order preflight, exact prepared-session replay, hidden-decision/rule/world/selection/council/camera drift attacks and atomic failure across every turn of all 46 routes; canonical speaker/Keeper occupancy, historical non-transcript disclosure, fictional cast classification, blocking/eyeline/lens identity and cast/dialogue/camera attacks; bounded wartable projection/camera/status/cycling/overlap rejection; exact live-signal values/order/anchoring/camera/cycling, active-oath and captured-terminal states; consequence order/timing/world closure/terminal failure, deterministic cut/ease authorship and semantic lens identity; source/claim/rights closure and hostile URL drift; every intermediate gameplay layer; save round-trip/tamper rejection; and procedural-audio contracts. Native target compilation, automation execution, PIE order/restart write plus figure/actor failure injection, council/site/signal/evidence/consequence interaction, audio-device capture/listening, final asset import, Linux packaging and human cinematic/playability review remain red until an official Epic installed build is available.
