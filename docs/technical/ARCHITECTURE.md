...
- Pointer picking plus `Tab`/Gamepad RB, `Shift+Tab` and `Home` focus the same inspected-site state. A non-mutating historical-basis mode merges scene + site only on current ground; a remote site exposes only its own evidence and is explicitly labeled intelligence rather than a destination. Private file paths and invented quotations are not packaged, and inspection cannot issue orders.
...
- Static repository validation is green. Unreal automation source covers bounded wartable projection/camera/status/cycling/overlap rejection, source/claim/rights closure and hostile URL drift, all 46 golden routes, every intermediate layer, save round-trip, tamper rejection, atomic failure behavior and procedural-audio contract checks. Native target compilation, automation execution, PIE marker/evidence interaction, audio-device capture/listening, asset import, Linux packaging and human cinematic/playability review remain red until an official Epic installed build is available.
