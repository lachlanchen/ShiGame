...
- Pointer picking plus `Tab`/Gamepad RB sites, `C`/Gamepad L3 signals, Shift reverse, `Home` current ground and `Space` camera skip focus one inspected world object without mutating campaign state. A non-mutating historical-basis mode merges scene + site only on current ground; a remote site exposes only its own evidence and is explicitly labeled intelligence rather than a destination. Signal focus restores current-site evidence scope, so gameplay/reconstruction state never masquerades as historical evidence. Private file paths and invented quotations are not packaged, and inspection cannot issue orders.
...
- The runtime builds a Daze command space with directional moonlight, motivated fire light, fog, ground and the five-site wartable. Site focus uses eased translation plus quaternion rotation; the short skippable order beat is evaluated from an explicit base transform and returns to it exactly, preventing cumulative drift. Engine basic-shape packages are explicitly cooked and selected-marker stencils are enabled.
...
- Static repository validation is green. Unreal automation source covers bounded wartable projection/camera/status/cycling/overlap rejection; exact live-signal values/order/anchoring/camera/cycling, active-oath and captured-terminal states, post-order refresh, cross-family spacing and atomic failure; source/claim/rights closure and hostile URL drift; all 46 golden routes and every intermediate layer; save round-trip/tamper rejection; and procedural-audio contracts. Native target compilation, automation execution, PIE site/signal/evidence interaction, audio-device capture/listening, asset import, Linux packaging and human cinematic/playability review remain red until an official Epic installed build is available.
