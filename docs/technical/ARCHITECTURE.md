...
- Schema v4 binds each playable node and wartable site to source records and inspectable historical/reconstruction claims, and adds a classified opponent model with complete, non-overlapping Exposure bands. Sites add a `known`/`reported`/`reference` intelligence state, bounded summary, uncertainty and schematic coordinates; claims retain locators, review state, confidence and game use without embedding source books.
- `scripts/validate-content.mjs` checks edition/right pairings, HTTPS public links, identifiers, claim/source/node/site closure, site coordinate/status bounds, reconstruction boundaries, translations, action/pressure/opposition/field effects, opponent-band coverage and reachability, requirements, cycles, deadlocks, every field-condition branch, real failure reachability, and all three conclusions.
...
Each chronicle records an unsigned 32-bit seed. FNV-1a over `campaignId|seed|nodeId|turn` selects one weighted authored field condition; the signal and exact effects are visible before commitment. Current Exposure independently selects a disclosed Qin-pursuit posture. The engine applies player effects, authored pressure effects, pursuit effects, then field effects and records all four stages separately before checking capture/scattering. Identical campaign data, seed and decision history therefore reconstruct the identical result in TypeScript and C#. Pursuit and field content is classified as dramatic reconstruction and cannot alter routes, requirements, flags or prose at runtime.
...
- Browser state is namespaced in `localStorage`. Save format 4 rebuilds state from decision history, preserves older decisions under their original rule contract with `legacyDecisionCount`, and rejects impossible, seed-inconsistent or posture-inconsistent sequences rather than trusting stored totals.
...
- PlayerPrefs stores the pre-alpha state in the same replayable save format 4 contract; production saves will later move to signed files without changing decision-history authority.
