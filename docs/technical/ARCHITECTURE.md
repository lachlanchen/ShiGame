...
Each chronicle records an unsigned 32-bit seed. FNV-1a over `campaignId|seed|nodeId|turn` selects one weighted authored field condition; the signal and exact effects are visible before commitment. Current Exposure independently selects a disclosed Qin-pursuit posture. Prior choices select a method read: fewer than two observations or a leading-count tie is neutral, while a unique leader prepares one counter that applies only if the pending choice uses its target method. The engine applies player effects, authored pressure, pursuit, method read and field condition in that order, recording all five layers separately before checking capture/scattering. Identical campaign data, seed and decision history therefore reconstruct the identical result in TypeScript and C#. Pursuit, method-read and field content is classified as dramatic reconstruction and cannot alter routes, requirements, flags or prose at runtime.
...
- Browser state is namespaced in `localStorage`. Save format 5 rebuilds state from decision history, uses `legacyDecisionCount` for the v1–v3 pre-pursuit boundary and `preMethodReadDecisionCount` for the v1–v4 pre-read boundary, and rejects impossible, seed-, posture-, method- or read-inconsistent sequences rather than trusting stored totals.
...
- PlayerPrefs stores the pre-alpha state in the same replayable save format 5 contract; production saves will later move to signed files without changing decision-history authority.
...
`scripts/validate-web-build.mjs` measures the built bytes with one deterministic gzip implementation and fails the build on regression. The current build is 99.15 KiB initial JavaScript, 9.58 KiB initial CSS, 2.33 KiB strategic map, 3.00 KiB opponent presentation, 7.41 KiB evidence/claim UI, 0.59 KiB field guide, 4.09 KiB detailed audio settings, 1.24 KiB audio engine, 0.93 KiB font loader and 178.95 KiB Three.js, all gzip. The complete 565-file Unicode-range font artifact is 22.94 MiB and the source-map-free deployable site is 26.78 MiB. That artifact size is not an initial player transfer: browser requests are limited by active locale, visible glyph ranges and invoked feature boundaries. Axe-core is development-only and does not enter the player bundle.
