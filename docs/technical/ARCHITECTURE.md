...
- The runtime builds a Daze command space with directional moonlight, motivated fire light, fog, ground and the five-site wartable. Site/signal focus uses eased translation plus quaternion rotation. After resolution and live-world refresh, `FShiCinematicBeatModel` atomically derives a stable order → optional oath → pressure → pursuit → method read → field → position sequence from actual intermediate records, exact final resources and live actor identities. The six-or-seven-beat sequence lasts 3.52–4.08 seconds, blocks unrelated commands, and `Space`/`Escape`/Gamepad B returns immediately to the authoritative current site without campaign mutation or cumulative camera drift. Engine basic-shape packages are explicitly cooked and selected-marker stencils are enabled.
...
- Static repository validation is green. Unreal automation source covers bounded wartable projection/camera/status/cycling/overlap rejection; exact live-signal values/order/anchoring/camera/cycling, active-oath and captured-terminal states, post-order refresh, cross-family spacing and atomic failure; consequence order/timing/world closure/non-mutation/terminal failure/atomic rejection after every turn of all 46 golden routes; source/claim/rights closure and hostile URL drift; every intermediate gameplay layer; save round-trip/tamper rejection; and procedural-audio contracts. Native target compilation, automation execution, PIE site/signal/evidence/consequence interaction, audio-device capture/listening, asset import, Linux packaging and human cinematic/playability review remain red until an official Epic installed build is available.
