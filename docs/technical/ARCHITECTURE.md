...
- A non-mutating historical-basis mode merges the active scene and wartable-site evidence boundaries. It presents classification, exact locator, public-edition origin, confidence, uncertainty and gameplay use; private file paths and invented quotations are not packaged, and opening the mode cannot issue orders.
...
- Static repository validation is green. Unreal automation source covers source/claim/rights closure and hostile URL drift, all 46 golden routes, every intermediate layer, save round-trip, tamper rejection, atomic failure behavior and procedural-audio contract checks. Native target compilation, automation execution, PIE play, evidence-link interaction, audio-device capture/listening, asset import, Linux packaging and human cinematic/playability review remain red until an official Epic installed build is available.
