...
Stage 3 Web-reference checkpoint, 2026-08-09: exact implementation `d872be2606918521cb91d5c0ceb9eb4ef6849a94` opens the shared engagement from the selected legal `broken-crossing` plan and current seeded condition. The player issues one legal command per pulse, sees player effects and the field's authored response as distinct events, reads six local meters and a three-entry command record, then receives an exact campaign-effect preview. Pointer, keyboard focus and standard X/Square → D-pad → A/Cross operation use the same pure resolver. The drawer makes the campaign stage inert, restores its exact invoker, supports Arabic RTL with clearly LTR source-copy fallback and does not write campaign history, node, resources or the save. The SHA-pinned visible run completes the best `cut-the-carts` route, checks the saved campaign byte-for-byte before/after and passes all 264 checks. This is a playable reference and rules witness, not the final spatial encounter and not campaign authority; Stage 4 native Unreal parity remains required.
