...
A localhost-only noVNC session visibly completed a mixed three-order route to **Costly success · Crossing under pressure**. All six pieces were simultaneously visible after a framing correction, the UI showed each player effect before its authored field answer, and the save hash remained identical before play, at outcome and after close. The archived Linux player then repeated the route with normal render/RHI threads and preserved packaged save SHA-256 `e9cbb4c857005fc04ac3dfe63b735296566655e0675e12c8045e8ccede2f42e8` at all three boundaries.

The official UE 5.8.1 Linux package completes successfully and the fixed-window normal-thread Vulkan player is stable for the accepted route. Its real-display FPS chart averages 195.18 FPS with zero hitches; Xvfb/noVNC remains presentation-limited and is used for visible review rather than performance acceptance. Editor PIE is still red: the first visible PIE attempt reached the real command space, then NVIDIA userspace failed on an outdated swapchain and crashed in `libnvidia-glcore.so.595.84`. Physical-controller review, native audio listening, injected failures, final environment/formation art and observed human playability remain open.
