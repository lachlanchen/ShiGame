"""Import or inspect SHI's reviewed Daze wet-field vegetation meshes.

The default mode is read-only inspection. Set SHI_DAZE_VEGETATION_REIMPORT=1
...
CONTRACTS = {
    "stalk": {
        "name": "SM_SHI_FieldStalkClump_01",
        "source": "stalk",
        "triangles": [84, 28],
        # Unreal's reviewed FBX -Z/Y conversion mirrors Blender Y.
        "minimum": [-34.0, -31.0, 0.0],
        "maximum": [34.0, 30.0, 135.0],
        "material": "M_SHI_RainDarkenedFieldPlant",
        "screen": 0.15,
    },
    "tuft": {
        "name": "SM_SHI_LowBladeTuft_01",
        "source": "tuft",
        "triangles": [64, 14],
        "minimum": [-45.0, -45.0, 0.0],
        "maximum": [45.0, 45.0, 52.0],
        "material": "M_SHI_RainDarkenedFieldPlant",
        "screen": 0.20,
    },
}
...
def main() -> None:
    ...
    replace = os.environ.get("SHI_DAZE_VEGETATION_REIMPORT") == "1"
    ...
    if not replace and any(existing) and not all(existing):
        raise RuntimeError("Daze vegetation destination is partial; explicit isolated reimport is required")
    ...
    report_path = project_dir / "Saved" / "Automation" / "shi-daze-wet-field-vegetation-unreal-import.json"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    unreal.log(f"SHI_DAZE_VEGETATION_REPORT {json.dumps(report, sort_keys=True)}")
    if not report["passed"]:
        raise RuntimeError(f"Daze vegetation Unreal admission failed: {report}")
