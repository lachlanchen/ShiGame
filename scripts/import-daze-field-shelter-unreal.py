"""Import or inspect SHI's reviewed Daze field-shelter blockout in Unreal.

The default mode is read-only inspection. Set SHI_DAZE_FIELD_SHELTER_REIMPORT=1
...
ASSET_NAME = "SM_SHI_DazeFieldShelter_01"
ASSET_PATH = f"{DESTINATION}/{ASSET_NAME}"
EXPECTED_MINIMUM = [-420.0, -336.7437, -18.0]
EXPECTED_MAXIMUM = [420.0, 336.7437, 337.0001]
EXPECTED_MATERIALS = {
...
def close_vector(actual: list[float], expected: list[float], tolerance: float = 0.10) -> bool:
...
def import_base_mesh(source: Path) -> tuple[unreal.StaticMesh, list[str], bool]:
    if unreal.EditorAssetLibrary.does_asset_exist(ASSET_PATH):
        if os.environ.get("SHI_DAZE_FIELD_SHELTER_REIMPORT") == "1":
...
def inspect_mesh(mesh: unreal.StaticMesh, imported_paths: list[str]) -> dict:
    ...
    checks = {
        ...
        "lodTriangles": lod_triangles == [3948, 460],
        "lodReduction": lod_count == 2 and lod_triangles[1] < lod_triangles[0] * 0.20,
        "lodScreenSizes": len(lod_screen_sizes) == 2
        and abs(lod_screen_sizes[0] - 1.0) <= 1e-5
        and abs(lod_screen_sizes[1] - 0.12) <= 1e-5,
        "materialSlots": {entry["slot"] for entry in material_slots} == EXPECTED_MATERIALS,
        "threeMaterials": len(material_slots) == 3 and all(entry["material"] for entry in material_slots),
        ...
        "commandSurfaceClearance": dimensions[0] > 800.0 and dimensions[1] > 640.0
        and minimum[2] <= -17.9 and maximum[2] <= 337.1,
...
def main() -> None:
    ...
    if imported_now:
        ...
        if not subsystem.set_lod_screen_sizes(mesh, [1.0, 0.12]):
    ...
    report_path = project_dir / "Saved" / "Automation" / "shi-daze-field-shelter-unreal-import.json"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    unreal.log(f"SHI_DAZE_FIELD_SHELTER_REPORT {json.dumps(report, sort_keys=True)}")
    if not report["passed"]:
        raise RuntimeError(f"Daze shelter Unreal admission failed: {report['checks']}")
