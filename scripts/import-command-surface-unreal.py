"""Import or inspect SHI's reviewed command-ground surface in Unreal.

The default mode is read-only inspection. Set SHI_COMMAND_SURFACE_REIMPORT=1
...
ASSET_NAME = "SM_SHI_CommandSurface_01"
ASSET_PATH = f"{DESTINATION}/{ASSET_NAME}"
EXPECTED_MINIMUM = [-290.0, -185.0, -2.0]
EXPECTED_MAXIMUM = [290.0, 185.0, 14.0]
...
def close_vector(actual: list[float], expected: list[float], tolerance: float = 0.06) -> bool:
...
def import_base_mesh(source: Path) -> tuple[unreal.StaticMesh, list[str], bool]:
    if unreal.EditorAssetLibrary.does_asset_exist(ASSET_PATH):
        if os.environ.get("SHI_COMMAND_SURFACE_REIMPORT") == "1":
...
def inspect_mesh(mesh: unreal.StaticMesh, imported_paths: list[str]) -> dict:
    ...
    checks = {
        ...
        "lodTriangles": lod_triangles == [620, 60],
        ...
        == {"M_SHI_WetPackedEarth", "M_SHI_DarkWorkedWood"},
        ...
        and int(mesh.get_editor_property("light_map_resolution")) == 128,
...
def main() -> None:
    ...
    report_path = project_dir / "Saved" / "Automation" / "shi-command-surface-unreal-import.json"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    unreal.log(f"SHI_COMMAND_SURFACE_REPORT {json.dumps(report, sort_keys=True)}")
    if not report["passed"]:
        raise RuntimeError(f"Command-surface Unreal admission failed: {report['checks']}")
