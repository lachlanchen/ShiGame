...
戦役グラフ、参照、翻訳キー、ルール、型、テストを自動検証します。可視noVNC/Chrome試験では遊び、史料分類、保存、RTL、モバイル、WebGL、コンソールを21項目確認しました。[証拠](../docs/production/PLAYTESTING.md)も公開しています。
...
2026-08-08 プレアルファ。Web章はビルド・可視試験済みです。実体のあるLinux/WebGL Unityエディタは導入され、Hubにも認識されています。ネイティブのインポートとコンパイルにはアカウント所有者によるログインとライセンス有効化が必要で、Unity 6の本番ピンは維持しています。両クライアント、史料、多言語、資産、プレイテスト、公開基準を満たすまで完成とはしません。
